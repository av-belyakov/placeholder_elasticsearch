...
func (em EventMessageTheHiveCase) ToStringBeautiful(num int) string {
	...
	strB.WriteString(fmt.Sprintf("%s'operation': '%s'\n", ws, em.Operation))
	strB.WriteString(fmt.Sprintf("%s'objectId': '%s'\n", ws, em.ObjectId))
	strB.WriteString(fmt.Sprintf("%s'objectType': '%s'\n", ws, em.ObjectType))
	strB.WriteString(fmt.Sprintf("%s'base': '%v'\n", ws, em.Base))
	strB.WriteString(fmt.Sprintf("%s'startDate': '%s'\n", ws, em.StartDate))
	strB.WriteString(fmt.Sprintf("%s'rootId': '%s'\n", ws, em.RootId))
	strB.WriteString(fmt.Sprintf("%s'requestId': '%s'\n", ws, em.RequestId))
	strB.WriteString(fmt.Sprintf("%s'organisationId': '%s'\n", ws, em.OrganisationId))
	strB.WriteString(fmt.Sprintf("%s'organisation': '%s'\n", ws, em.Organisation))
	strB.WriteString(fmt.Sprintf("%s'details':\n", ws))
	strB.WriteString(em.Details.ToStringBeautiful(num + 1))
	strB.WriteString(fmt.Sprintf("%s'object':\n", ws))
...
func (ed EventCaseDetails) ToStringBeautiful(num int) string {
	...
	strB.WriteString(fmt.Sprintf("%s'endDate': '%s'\n", ws, ed.EndDate))
	strB.WriteString(fmt.Sprintf("%s'resolutionStatus': '%s'\n", ws, ed.ResolutionStatus))
	strB.WriteString(fmt.Sprintf("%s'summary': '%s'\n", ws, ed.Summary))
	strB.WriteString(fmt.Sprintf("%s'status': '%s'\n", ws, ed.Status))
	strB.WriteString(fmt.Sprintf("%s'impactStatus': '%s'\n", ws, ed.ImpactStatus))
	strB.WriteString(fmt.Sprintf("%s'customFields': \n%s", ws, CustomFieldsToStringBeautiful(ed.CustomFields, num)))
...
func (eo EventCaseObject) ToStringBeautiful(num int) string {
	...
	strB.WriteString(fmt.Sprintf("%s'_id': '%s'\n", ws, eo.UnderliningId))
	strB.WriteString(fmt.Sprintf("%s'id': '%s'\n", ws, eo.Id))
	strB.WriteString(fmt.Sprintf("%s'createdBy': '%s'\n", ws, eo.CreatedBy))
	strB.WriteString(fmt.Sprintf("%s'updatedBy': '%s'\n", ws, eo.UpdatedBy))
	strB.WriteString(fmt.Sprintf("%s'createdAt': '%s'\n", ws, eo.CreatedAt))
	strB.WriteString(fmt.Sprintf("%s'updatedAt': '%s'\n", ws, eo.UpdatedAt))
	strB.WriteString(fmt.Sprintf("%s'_type': '%s'\n", ws, eo.UnderliningType))
	strB.WriteString(fmt.Sprintf("%s'caseId': '%d'\n", ws, eo.CaseId))
	strB.WriteString(fmt.Sprintf("%s'title': '%s'\n", ws, eo.Title))
	strB.WriteString(fmt.Sprintf("%s'description': '%s'\n", ws, eo.Description))
	strB.WriteString(fmt.Sprintf("%s'severity': '%d'\n", ws, eo.Severity))
	strB.WriteString(fmt.Sprintf("%s'startDate': '%s'\n", ws, eo.StartDate))
	strB.WriteString(fmt.Sprintf("%s'endDate': '%s'\n", ws, eo.EndDate))
	strB.WriteString(fmt.Sprintf("%s'impactStatus': '%s'\n", ws, eo.ImpactStatus))
	strB.WriteString(fmt.Sprintf("%s'resolutionStatus': '%s'\n", ws, eo.ResolutionStatus))
	strB.WriteString(fmt.Sprintf("%s'tags': \n%s", ws, ToStringBeautifulSlice(num, eo.Tags)))
	strB.WriteString(fmt.Sprintf("%s'flag': '%v'\n", ws, eo.Flag))
	strB.WriteString(fmt.Sprintf("%s'tlp': '%d'\n", ws, eo.Tlp))
	strB.WriteString(fmt.Sprintf("%s'pap': '%d'\n", ws, eo.Pap))
	strB.WriteString(fmt.Sprintf("%s'status': '%s'\n", ws, eo.Status))
	strB.WriteString(fmt.Sprintf("%s'summary': '%s'\n", ws, eo.Summary))
	strB.WriteString(fmt.Sprintf("%s'owner': '%s'\n", ws, eo.Owner))
