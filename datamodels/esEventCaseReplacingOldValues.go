...
// ReplacingOldValues заменяет старые значения структуры EventMessageForEsCase
// новыми значениями. Изменяемые поля:
// CommonEventType
// Details - детальная информация о событии
// Object - объект события
func (e *EventMessageForEsCase) ReplacingOldValues(element EventMessageForEsCase) (int, error) {
	var (
		err                  error
...
func (o *EventForEsCaseObject) ReplacingOldValues(element EventForEsCaseObject) int {
	var countReplacingFields int
